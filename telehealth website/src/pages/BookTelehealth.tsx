...
export default function BookTelehealth({ setPage }: BookProps) {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    email: '', address: '', state: '', postcode: '', medicare: '',
    contactMethod: 'Email', reason: '', symptoms: '', symptomStart: '',
              ...
              <div className="space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="date" className={inputClass} value={form.symptomStart} onChange={e => set('symptomStart', e.target.value)} />
                  </div>
                  <div>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <label className="flex items-center gap-3 p-4 rounded-xl border-2 border-dashed border-[#E2EBF6] cursor-pointer hover:border-[#0A6EBD]/50 transition-colors bg-[#F5F9FF]">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-[#0A1628]">Upload Photos</div>
                      <div className="text-xs text-[#94A3B8]">JPG, PNG up to 10MB</div>
                    </div>
                    <input type="file" accept="image/*" multiple className="hidden" />
                  </label>
                  <label className="flex items-center gap-3 p-4 rounded-xl border-2 border-dashed border-[#E2EBF6] cursor-pointer hover:border-[#0A6EBD]/50 transition-colors bg-[#F5F9FF]">
                    <svg className="w-6 h-6 text-[#0099A8]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/>
                      <path d="M14 2v6h6M16 13H8M16 17H8M10 9H8"/>
                    </svg>
                    <div>
                      <div className="text-sm font-semibold text-[#0A1628]">Upload Documents</div>
                      <div className="text-xs text-[#94A3B8]">PDF, DOC up to 20MB</div>
                    </div>
                ...
                {[
                  {
                    ...
                    fields: [
                      { label: 'Full Name', val: `${form.firstName} ${form.lastName}` || 'Emma Robertson' },
                      { label: 'Date of Birth', val: form.dob || '15/04/1989' },
                      { label: 'Gender', val: form.gender || 'Female' },
                      { label: 'Phone', val: form.phone || '0412 345 678' },
                      { label: 'Email', val: form.email || 'emma@example.com.au' },
                      { label: 'Address', val: `${form.address || '123 Collins St'}, ${form.state || 'VIC'} ${form.postcode || '3000'}` },
                    ],
                    step: 2,
                  },
                  {
                    ...
                    fields: [
                      { label: 'Reason', val: form.reason || 'General Consultation' },
                      { label: 'Symptoms', val: form.symptoms || 'Mild headache and fatigue for 3 days' },
                      { label: 'Pain Level', val: `${form.painLevel}/10` },
                      { label: 'Medications', val: form.medications || 'None' },
                      { label: 'Allergies', val: form.allergies || 'None known' },
                    ],
                    step: 3,
                  },
                  {
