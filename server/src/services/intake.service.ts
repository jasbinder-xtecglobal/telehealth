...
export class IntakeService {
  ...
  /**
   * The presenting-complaint options a booking may choose from.
   *
   * Served rather than duplicated in the client: the mapping from a
   * patient-facing label to a symptom category is a domain rule, and a second
   * copy in the UI is a second thing to keep in step.
   */
  bookingReasons() {
    return BOOKING_REASONS.map((r) => ({ value: r.value, label: r.label }));
  }

  /**
   * Books a consultation.
   *
   * Written for the public website; used from the workstation while that lives
   * in another repository. The rules are the patient's either way.
   *
